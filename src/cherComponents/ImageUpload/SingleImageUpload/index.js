...
function figureSize(size){
  ...
  let [w,h] = [size.width,size.height].sort((prev,next) => {
    return next - prev;
  });
...
function handleImageByCanvas(file, imgResource, size, callback){
  // console.log('4'); 
...
export default class ImageUpload extends React.Component {
  ...
  beforeUpload = (file, fileList) => {
    return new Promise((resolve) => {
      this.presetCanvas(file, function(dataurl){
  ...
  render(){
    ...
    const { size = "middle", children, ...props } = this.props;
